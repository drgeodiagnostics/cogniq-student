...
const getLayoutedElements = (rawData) => {
    ...
    // Space between nodes: [Height, Width]
    ...
    root.descendants().forEach((node) => {
        nodes.push({
            id: node.data.id,
            // Swap x and y to make it grow left-to-right instead of top-to-bottom
...
export default function InteractiveMindMap({ mapData }) {
    // 🚨 FALLBACK TO CHAPTER 1 DATA IF NONE PROVIDED
    ...
    const [activeNode, setActiveNode] = useState(activeData); // Default to root node
    ...
    return (
        <div className="w-full h-[70vh] bg-slate-50 dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden relative shadow-inner">
            ...
            >
                ...
                <Controls className="bg-white dark:bg-slate-800 shadow-lg rounded-lg border-none" />
                <MiniMap className="bg-white dark:bg-slate-800 rounded-lg shadow-lg border border-slate-200 dark:border-slate-700" maskColor="rgba(79, 70, 229, 0.1)" />
            </ReactFlow>

            {/* --- KNOWLEDGE HUB UI (Replaces your old HTML Sidebar) --- */}
            {activeNode && (
                <div className="absolute top-4 right-4 w-80 max-h-[90%] bg-white/90 dark:bg-slate-900/90 backdrop-blur-md rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 flex flex-col overflow-hidden animate-in slide-in-from-right-4 z-10">
                    <div className="bg-slate-100 dark:bg-slate-800 p-3 flex justify-between items-center border-b border-slate-200 dark:border-slate-700">
                        <span className="text-xs font-bold text-indigo-600 dark:text-indigo-400 uppercase tracking-widest flex items-center gap-2"><Info size={14}/> Knowledge Hub</span>
                    </div>
...
const STORAGE_BASE = "https://your-supabase-id.supabase.co/storage/v1/object/public/atlas-media/ch1/";
